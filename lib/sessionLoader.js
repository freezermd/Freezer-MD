 ...
 *   "CYPHER-X:~"   -> base64(zip archive containing creds.json)   [current]
 *   "FREEZER-MD~"  -> base64(gzip(creds.json))                    [legacy]
...
const KNOWN_PREFIXES = ['CYPHER-X:~', 'FREEZER-MD~', 'SILA-MD~', 'NEXUS___'];
...
/**
 * Normalizes a base64 string that may have been mangled in transit:
 *  - strips whitespace/newlines/zero-width characters that chat apps sometimes inject
 *  - accepts either the standard alphabet (+ /) or the URL-safe alphabet (- _)
...
function decodeSessionId(sessionId) {
    ...
    if (sessdata.includes('*')) {
        return {
            ok: false,
            reason:
                "the string contains '*', which is not a valid base64 character. This usually means it was " +
                'corrupted by WhatsApp\'s bold-text formatting during copy/paste. Regenerate the SESSION_ID and copy ' +
                'it from inside a code block (```like this```) instead of a plain message.',
        };
    }
 ...
 * matching CYPHER-X:~), for generating new session strings e.g. in a
 * "get session ID" pairing script. Uses STORE (uncompressed) to avoid
 * needing a deflate encoder here; decodeSessionId supports both.
 */
const CRC_TABLE = (() => {
...
function encodeSessionId(credsData, prefix = 'CYPHER-X:~') {
